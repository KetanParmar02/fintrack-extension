...
function mergePortfolios(remote: Portfolio[], local: Portfolio[]) {
    ...
    local.forEach((portfolio) => merged.set(portfolio.id ?? portfolio.name, portfolio))
    remote.forEach((portfolio) => merged.set(portfolio.id ?? portfolio.name, portfolio))
...
async function ensureSupabaseUserId() {
    ...
    if (!signInData.user?.id) throw new Error('Failed to get user id')
...
function App() {
    ...
    const createSamplePortfolio = async () => {
        ...
        try {
            ...
            setSyncMessage(`Saved locally only. Supabase error: ${errorMessage}`)
        } finally {
            setIsCreating(false)
        }
    }
    ...
    return (
        <div className="relative h-[580px] w-[420px] min-h-[580px] min-w-[420px] max-w-full overflow-hidden bg-[radial-gradient(circle_at_top,_rgba(16,185,129,0.14),_transparent_38%),linear-gradient(180deg,_#0f172a_0%,_#020617_100%)] text-white">
            ...
            <div
                className={`absolute inset-0 z-10 bg-slate-950/50 transition-opacity duration-300 ${isSidebarOpen ? 'opacity-100' : 'pointer-events-none opacity-0'}`}
            ...
            <div className="relative flex h-full flex-col p-2 sm:p-3">
                <header className="mb-2 flex items-center justify-between rounded-[24px] bg-slate-950/40 px-4 py-3 shadow-[0_12px_30px_rgba(2,6,23,0.25)] backdrop-blur-md">
                    ...
                    <button onClick={toggleSidebar} className="rounded-full bg-white/8 px-3 py-2 text-sm text-slate-200 transition hover:bg-white/12">
                        {isSidebarOpen ? 'Close menu' : 'Open menu'}
                    </button>
                </header>

                <aside className={`absolute left-2 top-2 bottom-2 z-20 flex w-[clamp(14.5rem,80vw,17rem)] flex-col overflow-hidden rounded-[28px] bg-slate-950/95 shadow-[0_24px_60px_rgba(2,6,23,0.55)] ring-1 ring-white/5 backdrop-blur-xl transition-transform duration-300 ease-out ${isSidebarOpen ? 'translate-x-0' : '-translate-x-[calc(100%+0.75rem)]'}`}>
                    <div className="flex items-center justify-between px-4 pt-4">
                        <button onClick={toggleSidebar} className="flex h-10 w-10 items-center justify-center rounded-2xl bg-emerald-500 text-lg font-bold text-white shadow-lg shadow-emerald-500/25 transition hover:scale-105">
                            F
                        </button>
                    ...
                    <nav className="mt-5 flex-1 space-y-2 px-3">
                        {sidebarItems.map((item) => (
                            <button
                                ...
                                className={`flex w-full items-center gap-3 rounded-2xl px-3 py-3 text-left transition-all ${activeTab === item.id ? 'bg-emerald-500/15 text-white ring-1 ring-emerald-400/25' : 'text-slate-300 hover:bg-white/5 hover:text-white'}`}
                            >
